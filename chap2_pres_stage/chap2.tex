\chapter{TEST Optimizing the performance of a centrifugal pump by data assimilation}
...
The internship began in February 2025. I was provided with a fix computer, and credentials to connect to the Irene and Cassiopée supercalcomputers. Marcello MELDI, Antoine DAZIN and Miguel MARTINEZ VALERO which are my internship supervisors, showed me around the lab...
Then, Tom MOUSSIE my and Miguel co-officies, explained to me the basics of OpenFoam and CONES, the DA code. I was able to run a few tests on OpenFoam on the supercomputers, which allowed me to familiarize myself with the tools and the code. % PARLER DE LA CONFIG DE CONES
...
\section{The open source code OpenFoam}

\dots


\section{The library ? CONES}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.42\textwidth]{images/ipsagarde.png}
...
\section{Some theoretical background on Data Assimilation}

L'interpolation consiste à définir, à partir d'un nombre fini de points, une fonction continue. En voici un exemple (Figure %\ref{fig:interpolation_cloche_points}) en \ac{1D}. L'axe x représente la position et l'axe y la valeur des points.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.495\textwidth}
        \centering
        \includegraphics[width=0.9\textwidth]{images/ipsagarde.png} % Dépasse un peu le minipage
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/ipsagarde.png}
...
\begin{comment}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\textwidth]{images/ipsagarde.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{images/ipsagarde.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{images/ipsagarde.png}
...
\section{Expetimentations}

\section{The pump}

Results in the file are High-Speed PIV data obtained at a PIV frequency of 980 Hz
...
\section{Preprocessing of PIV data}
...
\begin{comment}
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[scale=100]
            \centering
            % Axes
            \draw[->] (0,0) -- (0.09,0) node[right] {$x$ (m)};
            \draw[->] (0,0) -- (0,0.135) node[above] {$y$ (m)};
            
            % Zone d'acquisition
            \draw[fill=blue!20,opacity=0.5] (0,0) rectangle (0.08,0.124);
            \node at (0.04,0.062) {Zone PIV};
        
            % Graduation x
            \foreach \x in {0.02, 0.04, 0.06, 0.08}
            \draw[shift={(\x,0)},color=black] (0.0pt,1pt) -- (0pt,-0.1pt) node[below] {\x};
        
            % Graduation y
            \foreach \y in {0.02, 0.04, 0.06, 0.08, 0.10, 0.12}
            \draw[shift={(0,\y)},color=black] (0.1pt,0pt) -- (-0.1pt,0pt) node[left] {\y};
        
            % Ligne x = 0.0555 (theta = 0)
            \draw[red, very thick, dashed] (0.0555, 0) -- (0.0555, 0.124);
            \node[red, rotate=90] at (0.0555, 0.062) {$\theta = 0$};
        \end{tikzpicture}
    \end{figure}
\end{comment}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \draw[red, very thick, dashed] (0.0555, 0) -- (0.0555, 0.124);
        \node[red, rotate=90] at (0.053, 0.062) {$\theta = x_{abs} = 0$};
        
        \draw[->, >=stealth, color=red, thick] (0.0555, 0.0145) -- (0.07, 0.0145);
        \node[red, rotate=270] at (0.058, 0.0085) {$x_{abs}$};

        \draw[->, >=stealth, color=red, thick]  (0.0555, 0.0145) -- (0.0555, 0);
        \node[red, rotate=0] at (0.062, 0.017) {$y_{abs}$};
...
We find that between the line 55 and 56, the radius is constant and pass from lower than \(\pi\) to upper.
We also did plot x over theta for three differents values of theta and find out an intersection point for x = 0.0555.
So now, we know that for x = 0.0555, theta = \(\frac{\pi}{2}\)
...
Now, we keep only the data on the line x = 0.0555 (in fact we average data between x = 0.055 and x = 0.056).

Cones keep data in a particular way. We need to convert the data in a format that cones can read.
truncate at line 89, where the radius is upper than 0.3, because some reflexion on the pump



\section{Modifications of OpenFoam and CONES}
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/ipsagarde.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth]{images/ipsagarde.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\textwidth]{images/ipsagarde.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.31\textwidth}
        \centering
        \raisebox{0.5cm}{\includegraphics[width=\textwidth]{images/ipsagarde.png}}
        \caption{Base source}
        \label{fig:aac1}
    \end{subfigure}
    \hfill
    ...
    \begin{subfigure}[b]{0.31\textwidth}
        \centering
        \raisebox{0.0cm}{\includegraphics[width=\textwidth]{images/ipsagarde.png}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.67\textwidth]{images/ipsagarde.png}
