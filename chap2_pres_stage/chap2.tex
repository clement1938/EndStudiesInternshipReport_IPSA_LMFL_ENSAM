...
OK  Plan 0 repartir d'une simu convergé avec les nouveaux schémas (4e7 puis 5e7)

Plan a : plus raffiner autour de la sortie de la roue / entrée du diffuseur

Plan b : NONmodèle de turbulence k-epsilonNON paramètre de viscosité turbu de 0.09? to 0.5 (à augementer ?) dans momentumTransport (beta star ds kosst ? se renseigner)

Plan c a faire après: expension des valeurs (radial puis azimutale ? -> trouver une fonction approximative de la forme de la pale et la "convoluer" avec la fonction voulu) (selon karhunen loeve expansion) en majorant au niveau de la sortie de la roue

Plan n : ajouter la fuite (comment l'implémenter)

Passer à une simu "instationnaire" : URANS
