...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Evolution of the state through EnKF iterations (\includegraphics[width=0.014\textwidth]{images/cavity/legende.png}: observation point)}
\end{figure}
...
These high fidelity data are obtained from an experiment realised by Antoine Dazin\cite{dazinHighspeedStereoscopicPIV2011a} in 2011 at the LMFL. The experiment was realised on the SHF centrifugal pump. //RESEDA ?
\begin{figure}[H]
    \centering
...
(formalisme divisé par la masse volumique $\rho$ et $[\boldsymbol{u}] = L \cdot T^{-1}$ en incompressible).
...
When  $D \rightarrow \infty$, \\ \textbf{champ IBM = champ body-fitted }
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{images/numVSxp.png}
    \caption{implicit forcing term in OpenFoam}
    \label{numVSxp}
\end{figure}
...
\subsection{Simple application of the EnKF}
...
\subsection{More sofisticated application of the EnKF}
