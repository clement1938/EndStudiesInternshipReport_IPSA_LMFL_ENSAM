...
\PassOptionsToPackage{pdfmanagement-testphase=false}{hyperref}
...
\usepackage[utf8]{inputenc}          % encodage (OK si pas en XeLaTeX ou LuaLaTeX)
\usepackage[hyphens]{url}           % permet le retour à la ligne dans les URL
\usepackage[hidelinks, breaklinks=true]{hyperref}  % doit venir APRÈS url
\urlstyle{same}                     % garde l'apparence normale des URL
\usepackage{tablefootnote}          % si tu veux faire des notes dans les tableaux
...
%  declare toks={file icon}{\includegraphics[height=1em]{images/file.png}},
...
\newcommand{\annexref}[1]{Appendix~\ref{#1}}
...
\newcommand{\annex}[2]{%
  ...
  \section*{Appendix \theannex\ -- #1}%
  \label{#2}%
  \addcontentsline{toc}{section}{Appendix \theannex\ -- #1}
}
...
\DeclareAcronym{PIMPLE}{
  short = PIMPLE,
  long = merged PISO-SIMPLE algorithm,
  short-indefinite = une,
}

\DeclareAcronym{PISO}{
  short = PISO,
  long = Pressure Implicit with Splitting of Operators,
  short-indefinite = une,
}
...
\acresetall % Permet de redonner la définition des acronymes avec \ac{id}

\input{chap2_establisment/chap2.tex}
\newpage

\input{chap3_application/chap3.tex}
\newpage
...
\chapter*{Appendices table}
\addcontentsline{toc}{chapter}{Appendices table}

\begin{itemize}
    \item \textbf{Appendix 1} - Discretization and coupling in OpenFOAM (RANS / U\,-RANS) \dotfill \pageref{annex:OpenFOAMRANS}
    \item \textbf{Appendix 2} - OpenFOAM Directory Tree \dotfill \pageref{annex:OpenFoamArbo}
    \item \textbf{Appendix 3} - Extract of conesDict file \dotfill \pageref{annex:conesDict}
    \item \textbf{Appendix 4} - Batch code used to run simulation on the Rome partition of TGCC cluster \dotfill \pageref{annex:BatchCode}
    % Ajoute ici les autres annexes au fur et à mesure  % \annex{Discretization and coupling in OpenFOAM (RANS / U\,-RANS)}{annex:OpenFOAMRANS}
\end{itemize}
