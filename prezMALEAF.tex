...
\title[Projet MALEAF]{\structure{Assimilation de données pour une pompe centrifuge}}


\author[C. Thibault, M.M. Valero]{Clément THIBAULT\textsuperscript{\dag}, Miguel M. VALERO\textsuperscript{\dag}}%, Marcello MELDI\textsuperscript{\dag}}
...
\begin{block}{\structure{\textbf{Penalisation IBM:}}}
    Pénalisation continue de type IBM pour les écoulements incompressibles $\Rightarrow$ définition d’un \structure{terme de forçage direct local} $\boldsymbol{f}_P$ dans les équations de Navier–Stokes, afin d’\textbf{imposer les conditions aux limites sur la paroi immergée} $\boldsymbol{(ib)}$.
\end{block}
...
\begin{columns}
    ...
    \begin{itemize}[<+->]
        \item Simulation stationnaire avec la l'approche \structure{\textbf{Multiple Reference Frame (MRF)}}
...
\begin{frame}{Modélisation IBM}
    ...
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item \structure{\textbf{Performance du diffuseur expérimentale et numérique similaire mais différente}}
...
\begin{frame}{DA avec l'EnKF - Premiers résultats en stationaire}
...
\begin{columns}
  ...
  \column{0.48\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=0.6\linewidth]{newFigures/assymetrie.png}
            \caption{Champs de vitesse absolue pour\\
            Dx = 4e7 et Dy = 2e7}
        ...
        \column{0.48\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.6\linewidth]{newFigures/coords.png}
